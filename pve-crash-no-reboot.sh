...
printf 'Автоматическая перезагрузка отключается (panic=0); резервные копии не создаются.\n'
...
printf '\nДля продолжения введите ровно «CRASH NO REBOOT»: '
read -r confirmation
[[ "$confirmation" == 'CRASH NO REBOOT' ]] ||
    die 'подтверждение не совпало; паника не вызвана.'

printf 'Отключается автоматическая перезагрузка и вызывается паника...\n'
